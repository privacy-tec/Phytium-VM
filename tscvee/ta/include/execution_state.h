...
{
    IMSG("Error: out of memory.");
    // In OP-TEE, we should handle this differently
}

void Memory_allocate_capacity(struct Memory *memory)
{
    memory->m_data = (uint8_t*)TEE_Realloc(memory->m_data, memory->m_capacity);
